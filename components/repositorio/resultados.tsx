    ...
    <div className="min-w-0">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3.5">
        ...
        <div className="relative flex items-center">
          <Search
            className="pointer-events-none absolute left-3 h-[15px] w-[15px] text-[#8ca0af]"
            aria-hidden
          />
          ...
          <input
            ...
            className="w-56 rounded-lg border border-linea bg-white py-2.5 pl-8.5 pr-3 text-sm outline-none transition-shadow focus:border-andritz focus:ring-2 focus:ring-andritz/15"
          />
        </div>
      </div>

      <p className="mb-4 font-mono text-xs text-tinta-suave">{metaResultados}</p>
      ...
      {n > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-[#c4d0d9] bg-white p-12 text-center">
          <span className="inline-flex h-13 w-13 items-center justify-center rounded-2xl bg-[#eef4f8] text-[#8ca0af]">
...
function TarjetaMaterial({
  ...
  return (
    <Link
      ...
      className="group flex h-full flex-col rounded-2xl border border-linea bg-white p-5.5 shadow-[0_12px_30px_-26px_rgba(12,42,62,0.5)] transition hover:-translate-y-[3px] hover:border-andritz hover:shadow-[0_20px_40px_-28px_rgba(12,42,62,0.55)]"
    >
      <div className="flex items-center gap-3">
        <span className="flex h-11 w-11 flex-none items-center justify-center rounded-[11px] bg-[#eef4f8] text-andritz">
      ...
      <h3 className="mt-4 font-display text-lg font-semibold leading-snug group-hover:text-andritz">
        {material.titulo}
      </h3>
      <p className="mt-2 flex-1 text-sm leading-relaxed text-tinta-suave">
        {material.resumen}
      </p>
      <div className="mt-4.5 flex items-center justify-between gap-3 border-t border-[#eef2f5] pt-4">
        <span className="flex items-center gap-1.5 font-mono text-[11px] text-[#6a7f8e]">
          <List className="h-[13px] w-[13px]" strokeWidth={1.9} aria-hidden />
          {nSecciones} {nSecciones === 1 ? "sección" : "secciones"}
        </span>
        <span className="flex items-center gap-1.5 font-mono text-[11px] font-medium uppercase tracking-wide text-andritz">
          Abrir
          <ArrowRight
